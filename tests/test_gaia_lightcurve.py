...
def test_gaia_id():
    ...
    assert fetched_id == '2154100169676165120', 'Incorrect ID returned for \
        NQ Dra, Check function'


def test_gaia_id_null():
    """
    Unit test to evaluates the correctness of the gaia_id() function using a \
...
def test_fetch_lightcurve_dr2():
    """
    Unit test to evaluates the correctness of the fetch_lightcurve_dr2() \
    function
    """
    ...
    print(len(lc))
    assert len(
        lc) == 84, "Unexpected length of the Lightcurve fetched from DR2, \
            check function"


def test_fetch_lightcurve_dr1():
    """
    Unit test to evaluates the correctness of the fetch_lightcurve_dr1() \
    function
    """
    ...
    print(len(lc))
    assert len(
        lc) == 144, "Unexpected length of the Lightcurve fetched from DR1, \
            check function"
